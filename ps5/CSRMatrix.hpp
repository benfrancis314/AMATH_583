...
// WHAT THIS WOULD LOOK LIKE, WITH RACE CONDITIONS
  ...
  void matvec(const Vector& x, Vector& y, size_t partitions) const {
    ...
    for (size_t i = 0; i < partitions; ++i) {
      futures_.push_back(std::async(std::launch::async, &CSRMatrix::worker_a, this, std::cref(x), std::ref(y), i*blocksize, (i+1)*blocksize));
    }
  ...
  //   for (size_t i = 0; i < partitions; ++i) {
  //     futures_.push_back(std::async(std::launch::async, &CSRMatrix::worker_b, this, std::cref(x), std::ref(y), i*blocksize, (i+1)*blocksize));
  //   }
