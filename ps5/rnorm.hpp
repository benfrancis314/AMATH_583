...
double recursive_worker_a(const Vector& x, size_t begin, size_t end, size_t level) { 
  if (level == 1) {
    ...
    std::future<double> left = std::async(std::launch::async, recursive_worker_a, std::cref(x), begin, begin+(end-begin)/2, level/2);
    std::future<double> right = std::async(std::launch::async, recursive_worker_a, std::cref(x), begin+(end-begin)/2, end, level/2);
...
double recursive_worker_b(const Vector& x, size_t begin, size_t end, size_t level) { 
  if (level == 1) {
    ...
    std::future<double> left = std::async(std::launch::deferred, recursive_worker_a, std::cref(x), begin, begin+(end-begin)/2, level/2);
    std::future<double> right = std::async(std::launch::deferred, recursive_worker_a, std::cref(x), begin+(end-begin)/2, end, level/2);
