...
// WHAT THIS WOULD LOOK LIKE, WITH RACE CONDITIONS
  ...
  void worker_b(const Vector& x, Vector& y, size_t begin, size_t end) const {
    for (size_t i = begin; i < end; ++i) {
      for (size_t j = col_indices_[i]; j < col_indices_[i + 1]; ++j) {
...
// WHAT THIS WOULD LOOK LIKE, WITH RACE CONDITIONS
  ...
  //   size_t blocksize = num_cols_ / partitions;

  //   for (size_t i = 0; i < partitions; ++i) {
  //     futures_.push_back(std::async(std::launch::async, &CSCMatrix::worker_a, this, std::cref(x), std::ref(y), i*blocksize, (i+1)*blocksize));
  //   }
  ...
  void t_matvec(const Vector& x, Vector& y, size_t partitions) const {
    ...
    for (size_t i = 0; i < partitions; ++i) {
      futures_.push_back(std::async(std::launch::async, &CSCMatrix::worker_b, this, std::cref(x), std::ref(y), i*blocksize, (i+1)*blocksize));
    }
