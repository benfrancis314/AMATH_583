...
#include "CImg.h" // Image processing library for C
using namespace cimg_library; // NOTE the "using" namespace; for "save"

#include "Timer.hpp"

#inclusde "AOS_Image.hpp"
...
int main(int argc, char* argv[]) {
  ...
  std::string  in_image = argv[1]; // Get string of image name
  CImg<double> img(in_image.c_str()); // Read image

  SOA_Image    soa_img    = to_soa(img); // Store in SOA
  AOS_Image    aos_img    = to_aos(img); // Store in AOS
  Tensor_Image tensor_img = to_tensor(img); // Store in Tensor
