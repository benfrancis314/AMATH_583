...
auto scatter_images(const std::string& bin_file, size_t num_images, size_t height, size_t width) {
  ...
  // This just divvies out the remainder
  ...
  if (myrank == 0) {
    ...
    if (mysize > 1) { // If more than one node available
      size_t count = sendcounts[1]; // starting after the root node
      for (size_t rank = 1; rank < mysize; ++rank) { // go through all nodes
        for (size_t i = 0; i < sendcounts[rank]; ++i) { // How many images should this node get? sendcounts[rank] is total, i iterates thru
          // Send z[count]
          // Write me
          MPI::COMM_WORLD.Send(&z[count](0), buffer_size, MPI::DOUBLE, rank, 0);
          ++count; // Iterates through database
        }
      }
    }
    ...
    for (size_t i = 0; i < sendcounts[myrank]; ++i) {
      ...
      // Write me
      MPI::COMM_WORLD.Recv(&myimg_data[i](0), buffer_size, MPI::DOUBLE, 0, 0);
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  MPI::COMM_WORLD.Reduce(&local_C(0,0), &C(0,0), local_C.num_rows() * local_C.num_cols(), MPI::FLOAT, MPI::SUM, 0);
